...
set -uo pipefail
D="${1:-}"; [[ -n "$D" ]] || { echo "用法: $0 <device-serial>"; exit 2; }
APP=com.shareinstalls.demo_app
A="adb -s $D"
API=http://localhost:6066/api
PAGE=http://localhost:6066/examples/web/fingerprint-demo.html
...
$A reverse tcp:6066 tcp:6066 >/dev/null
# URL 必须在设备侧加引号：& 会被设备 shell 当成后台运算符截断，code/auto 静默丢失。
# 也不要 force-stop Chrome，MIUI 之后不再允许 intent 把它拉起。
$A shell "am start -a android.intent.action.VIEW -n com.android.chrome/com.google.android.apps.chrome.Main --ez create_new_tab true -d '$PAGE?api=$API&code=$CODE&auto=1'" >/dev/null 2>&1
sleep 12
...
for _ in $(seq 1 20); do sleep 1; $A shell dumpsys activity activities 2>/dev/null | grep -q "topResumedActivity.*$APP" && break; done
